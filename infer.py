...
_FALLBACK_MODEL_CFG = {
    ...
    'use_target_history_matching': False,
    'use_user_dense_groups': False,
    'use_din_residual': False,
    'din_top_k': 80,
    'use_dcn_residual': False,
    'dcn_rank': 16,
    'dcn_layers': 2,
    'use_aux_logit_residual': False,
...
def _batch_to_model_input(
    ...
    sample_day_id = device_batch.get(
        'day_of_week',
        torch.zeros(B, dtype=torch.long, device=device))
    sample_day_id = sample_day_id.long().clamp(0, 7)
    raw_hour = device_batch.get('hour')
    if raw_hour is None:
        sample_hour_id = torch.zeros(B, dtype=torch.long, device=device)
    else:
        sample_hour_id = (raw_hour.long() + (sample_day_id > 0).long()).clamp(0, 24)
    return ModelInput(
        ...
        sample_hour_id=sample_hour_id,
        sample_hour_sin=device_batch.get(
            'hour_sin',
            torch.zeros(B, dtype=torch.float32, device=device)),
        sample_hour_cos=device_batch.get(
            'hour_cos',
            torch.zeros(B, dtype=torch.float32, device=device)),
    )
