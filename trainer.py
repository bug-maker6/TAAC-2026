...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        self.save_epoch_ckpt: int = int(save_epoch_ckpt)
        if use_bf16 is None:
            use_bf16 = os.environ.get('USE_BF16', '0').lower() in ('1', 'true', 'yes', 'on')
        self.use_bf16: bool = bool(use_bf16)
        self.autocast_enabled: bool = bool(
            self.use_bf16 and str(device).startswith('cuda') and torch.cuda.is_available()
        )
        if self.use_bf16 and not self.autocast_enabled:
            logging.warning(
                "bf16 requested but CUDA is unavailable / current device is not CUDA; "
                "falling back to fp32"
            )

        self.ema_decay: float = float(ema_decay)
        self.ema_start_step: int = int(ema_start_step)
        self.ema_update_every: int = max(1, int(ema_update_every))
        self.eval_raw_and_ema: bool = bool(eval_raw_and_ema)
        self.ema_enabled: bool = self.ema_decay > 0.0
        self.ema_shadow: Dict[str, torch.Tensor] = {}
        self.ema_updates: int = 0
        if self.ema_enabled:
            self._init_ema_shadow()

        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     f"focal_alpha={focal_alpha}, focal_gamma={focal_gamma}, "
                     f"reinit_sparse_after_epoch={reinit_sparse_after_epoch}, "
                     f"use_bf16={self.use_bf16}, autocast_enabled={self.autocast_enabled}, "
                     f"ema_decay={self.ema_decay}, ema_start_step={self.ema_start_step}, "
                     f"eval_raw_and_ema={self.eval_raw_and_ema}")

    def _autocast_context(self) -> Any:
        """Return a CUDA bf16 autocast context when enabled, else a no-op."""
        if not self.autocast_enabled:
            return nullcontext()
        return torch.autocast(device_type='cuda', dtype=torch.bfloat16)

    def _init_ema_shadow(self) -> None:
        """Initialize EMA over dense parameters only.

        Sparse embedding tables are intentionally excluded: duplicating them can
        be very expensive, and dense-parameter EMA already smooths the backbone,
        fusion heads, and classifier where most non-id generalization happens.
        """
        sparse_ptrs = set()
        ...
        for name, param in self.model.named_parameters():
            ...
            tracked_params += param.numel()
            tracked_tensors += 1
        logging.info(
            f"EMA initialized over dense params only: "
            f"{tracked_tensors} tensors, {tracked_params:,} parameters"
        )

    def _update_ema(self, global_step: int) -> None:
        """Update dense-parameter EMA after an optimizer step."""
        if (
            not self.ema_enabled
            or global_step < self.ema_start_step
            or global_step % self.ema_update_every != 0
            or not self.ema_shadow
        ):
            return
    ...
    @contextmanager
    def _ema_scope(self) -> Any:
        """Temporarily swap dense parameters to their EMA values."""
        if not self.ema_enabled or not self.ema_shadow or self.ema_updates <= 0:
            yield
            return
    ...
    def _build_step_dir_name(
        ...
        ``global_step2500.layer=2.head=4.hidden=64[.epochN][.best_model]``.
        """
    ...
    def _save_step_checkpoint(
        ...
        Args:
            ...
            extra_tag: optional suffix appended to the checkpoint directory
                name, e.g. ``epoch8`` for a fixed-epoch checkpoint.

        Returns:
            The absolute path of the checkpoint directory.
        """
        dir_name = self._build_step_dir_name(
            global_step, is_best=is_best, extra_tag=extra_tag)
    ...
    def _save_fixed_epoch_checkpoint(
        self,
        epoch: int,
        total_step: int,
        use_ema: bool = False,
    ) -> None:
        """Save an epoch checkpoint at the configured interval.

        ``save_epoch_ckpt=1`` means save every epoch, ``2`` every two epochs,
        and ``0`` disables epoch snapshots.
        """
        if self.save_epoch_ckpt <= 0 or epoch % self.save_epoch_ckpt != 0:
            return
        extra_tag = f"epoch{epoch}.ema" if use_ema else f"epoch{epoch}"
        ckpt_dir = os.path.join(
            self.save_dir,
            self._build_step_dir_name(total_step, extra_tag=extra_tag),
        )
        if os.path.exists(os.path.join(ckpt_dir, "model.pt")):
            logging.info(f"Epoch checkpoint already exists, skip: {ckpt_dir}/model.pt")
            return
        if use_ema:
            with self._ema_scope():
                self._save_step_checkpoint(total_step, extra_tag=extra_tag)
        else:
            self._save_step_checkpoint(total_step, extra_tag=extra_tag)
        logging.info(f"Saved epoch checkpoint for epoch {epoch}: {ckpt_dir}/model.pt")

    def _evaluate_for_selection(self, epoch: Optional[int] = None) -> Dict[str, Any]:
        """Evaluate raw weights and optionally EMA weights, then pick higher AUC."""
        raw_auc, raw_logloss = self.evaluate(epoch=epoch, eval_name='raw')
        result: Dict[str, Any] = {
            'selected': 'raw',
            'auc': raw_auc,
            'logloss': raw_logloss,
        ...
        if self.eval_raw_and_ema and self.ema_enabled and self.ema_updates > 0:
            ema_auc, ema_logloss = self.evaluate(
                epoch=epoch,
                use_ema=True,
                eval_name='ema',
            )
            result['ema_auc'] = ema_auc
            result['ema_logloss'] = ema_logloss
            # AUC is the primary objective. For an exact tie, prefer EMA only
            # when it also improves logloss.
            if (
                ema_auc > raw_auc
                or (ema_auc == raw_auc and ema_logloss <= raw_logloss)
            ):
                result['selected'] = 'ema'
                result['auc'] = ema_auc
                result['logloss'] = ema_logloss

        logging.info(
            "Validation selection | selected=%s AUC=%s LogLoss=%s "
            "| raw=(%s, %s) ema=(%s, %s) ema_updates=%s",
            result['selected'],
            result['auc'],
            result['logloss'],
            result['raw_auc'],
    ...
    def _handle_selected_validation_result(
        self,
        total_step: int,
        eval_result: Dict[str, Any],
    ) -> None:
        """Save/early-stop using the selected raw or EMA weight set."""
        if eval_result['selected'] == 'ema':
            with self._ema_scope():
                self._handle_validation_result(
                    total_step,
                    float(eval_result['auc']),
                    float(eval_result['logloss']),
                )
        else:
            self._handle_validation_result(
                total_step,
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for step, batch in train_pbar:
                ...
                if self.eval_every_n_steps > 0 and total_step % self.eval_every_n_steps == 0:
                    ...
                    eval_result = self._evaluate_for_selection(epoch=epoch)
                    val_auc = float(eval_result['auc'])
                    val_logloss = float(eval_result['logloss'])
                    self.model.train()
                    torch.cuda.empty_cache()

                    logging.info(
                        f"Step {total_step} Validation | selected={eval_result['selected']} "
                    ...
                    if self.writer:
                        ...
                        self.writer.add_scalar('AUC/valid_raw', eval_result['raw_auc'], total_step)
                        self.writer.add_scalar('LogLoss/valid_raw', eval_result['raw_logloss'], total_step)
                        if eval_result['ema_auc'] is not None:
                            self.writer.add_scalar('AUC/valid_ema', eval_result['ema_auc'], total_step)
                            self.writer.add_scalar('LogLoss/valid_ema', eval_result['ema_logloss'], total_step)

                    self._handle_selected_validation_result(total_step, eval_result)
            ...
            logging.info(
                f"Epoch {epoch} Validation | selected={eval_result['selected']} "
                f"AUC: {val_auc}, LogLoss: {val_logloss}"
            )
            ...
            self._save_fixed_epoch_checkpoint(
                epoch,
                total_step,
                use_ema=(eval_result['selected'] == 'ema'),
            )
            self._handle_selected_validation_result(total_step, eval_result)
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        return ModelInput(
            ...
            sample_day_id=device_batch.get('sample_day_id', torch.zeros(B, dtype=torch.long, device=self.device)),
            sample_hour_id=device_batch.get('sample_hour_id', torch.zeros(B, dtype=torch.long, device=self.device)),
            sample_hour_sin=device_batch.get('sample_hour_sin', torch.zeros(B, dtype=torch.float32, device=self.device)),
            sample_hour_cos=device_batch.get('sample_hour_cos', torch.zeros(B, dtype=torch.float32, device=self.device)),
        )
    ...
    def evaluate(
        ...
        print(f"Start Evaluation (PCVRHyFormer) - {eval_name}")
        self.model.eval()
        if not epoch:
            epoch = -1
        ...
        with torch.no_grad():
            for step, batch in pbar:
                ...
                # bf16 CPU tensors cannot be converted to numpy directly;
                # keep metric computation in fp32.
